...
def matmul(a: torch.Tensor, b: torch.Tensor):
    """Float32 matmul for numerical stability — TF32 disabled to match kernel IEEE float32 FMA."""
    # On Ampere, PyTorch defaults to TF32 (10-bit mantissa) for float32 matmul.
    # The CUDA kernel uses IEEE float32 fmaf. After 8-9 sequential token updates,
    # TF32 vs IEEE rounding accumulates up to ~4-5 absolute error in state values.
    # Disabling TF32 here makes the reference match the kernel.
...
def run(q, k, v, state, A_log, a, dt_bias, b, cu_seqlens, scale):
    ...
    assert num_q_heads == 4, f"Expected num_q_heads=4, got {num_q_heads}"
    assert num_k_heads == 4, f"Expected num_k_heads=4, got {num_k_heads}"
    assert num_v_heads == 8, f"Expected num_v_heads=8, got {num_v_heads}"
    assert head_size == 128, f"Expected head_size=128, got {head_size}"
